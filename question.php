...
<?php
    ...
    if( isset($_POST['answer']) ){
        $prev_answer = strtolower($_POST['answer']);
    }
    ...
    if( $cur_questionID > $question_count ){
        ...
        $row = mysqli_fetch_assoc($result);
        $imgurl = $row['ImageURL'];
        $questionText = nl2br($row['QuestionText']);
        $ans = nl2br($row['Answer']);
        echo " <img class='question-img' src='".$imgurl."' >";
        echo " <p class='question-txt'>".$questionText."</p>";
        echo "<form id='answer-form' action='question.php' method='post'>";
        echo "<input id='answer-field' class='glowing-border'  name='answer' type='password' placeholder='Answer' >";
